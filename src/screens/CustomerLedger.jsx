// src/components/CustomerLedger.jsx
...
import LedgerPDFTemplate from '../components/LedgerPDFTemplate';  // ✅ Correct
import { exportLedgerToPDF, formatIndianCurrency } from '../utils/pdfExport';  // ✅ Correct
...
const CustomerLedger = ({ customer, onBack }) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const closingBalance = filteredEntries[0]?.runningBalance || openingBalance;
      ...
      const success = await exportLedgerToPDF(
        pdfData,
        `Customer_Ledger_${customer.name.replace(/[^a-zA-Z0-9]/g, '_')}_${exportStart}_to_${exportEnd || 'current'}.pdf`
      );
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fffde7, #e3f2fd)', padding: '24px' }}>
      <div style={{ 
        ...
        <div style={{
          ...
          <h4 style={{ margin: '0 0 12px 0', color: '#1976d2' }}>📊 Export Ledger to PDF</h4>
